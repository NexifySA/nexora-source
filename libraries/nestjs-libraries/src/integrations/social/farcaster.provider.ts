  ...
  implements SocialProvider
{
  ...
  async post(
    ...
    for (const channel of channels) {
      const data = await client.publishCast({
        // Cast a any: @neynar/nodejs-sdk endurecio el tipo
        // PostCastReqBodyEmbeds (ahora requiere cast_id/castId aunque la API
        // sigue aceptando { url }). Runtime no cambia.
  ...
  async comment(
    ...
    for (const parentHash of parentIds) {
      const data = await client.publishCast({
        // Mismo cast a any que en post() — ver explicacion arriba.
