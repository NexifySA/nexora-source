...
export class TiktokProvider extends SocialAbstract implements SocialProvider {
  ...
  convertToJPEG = true;
  // Aligned with the TikTok app's actual approved products:
  // Login Kit (user.info.basic) + Content Posting API with Direct Post ON
  // (video.upload + video.publish). Optional analytics scopes
  // (video.list, user.info.profile, user.info.stats) are NOT requested
  // because they are not enabled on the app.
  scopes = [
    'user.info.basic',
    'video.upload',
    'video.publish',
  ...
  override handleErrors(body: string):
    ...
    if (body.indexOf('url_ownership_unverified') > -1) {
      return {
        type: 'bad-body' as const,
        value: 'You have to upload the picture/video to Nexpost when sending a URL',
      };
    }
  ...
  async authenticate(params: {
    ...
    if (tokenResponse?.error || !tokenResponse?.access_token) {
      ...
      throw new NotEnoughScopes(`TikTok token exchange failed: ${reason}`);
    }
    ...
    // Only request fields available with our approved scopes:
    //   user.info.basic   -> open_id, avatar_url, display_name, union_id
    //   user.info.profile -> bio_description, profile_deep_link, is_verified, username
    //   user.info.stats   -> follower_count, following_count, likes_count, video_count
    // Since we only have user.info.basic, requesting username/etc. causes a
    // scope error that throws here and surfaces as "Authentication failed".
    ...
    console.log('[tiktok] user info response', {
      hasUser: !!userInfoResp?.data?.user,
      error: userInfoResp?.error,
      keys: Object.keys(userInfoResp || {}),
    });

    if (userInfoResp?.error?.code && userInfoResp.error.code !== 'ok') {
      throw new NotEnoughScopes(
        `TikTok /user/info failed: ${userInfoResp.error.message || userInfoResp.error.code}`
      );
    }

    const user = userInfoResp?.data?.user || {};
    const open_id: string = user.open_id || '';
    const display_name: string = user.display_name || '';
    const avatar_url: string = user.avatar_url || '';

    if (!open_id) {
      throw new NotEnoughScopes('TikTok /user/info returned no open_id');
    }

    return {
      id: open_id.replace(/-/g, ''),
      name: display_name || `Channel_${open_id.slice(0, 8)}`,
      accessToken: access_token,
      refreshToken: refresh_token,
      expiresIn: dayjs().add(23, 'hours').unix() - dayjs().unix(),
      picture: avatar_url,
