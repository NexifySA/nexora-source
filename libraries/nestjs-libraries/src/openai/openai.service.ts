...
// Nexpost MVP: AI deshabilitada por defecto.
// Sin OPENAI_API_KEY real, no se instancia el cliente; cualquier llamada
// tira un error controlado en lugar de crashear runtime con un key dummy.
...
function requireOpenai(): OpenAI {
  if (!AI_ENABLED || !openai) {
    throw new Error(
      'AI features are disabled. Set AI_ENABLED=true and OPENAI_API_KEY to enable.'
    );
  }
  return openai;
}
