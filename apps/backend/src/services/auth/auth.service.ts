...
export class AuthService {
  ...
  /** Sembrar trial automatico de 14 dias en signup. Idempotente. */
  private async _seedTrialSafe(organizationId: string) {
    try {
      await this._subscriptionService.seedTrial(organizationId);
    } catch (err: any) {
      // No abortar el registro si la seed falla — pero loguear el error
      // COMPLETO (stack incluido) para diagnostico.
      console.error(
        `[auth] seedTrial fallo org=${organizationId} error=${
          err?.message || err
        }`
      );
  ...
  async routeAuth(
    ...
    if (provider === Provider.LOCAL) {
      ...
      if (body instanceof CreateOrgUserDto) {
        ...
        await this._seedTrialSafe((create as any).id);
